...
<header
    ...
    <div class="position-relative logo-bar-area border-md-nonea z-1025">
        <div class="container">
           <div class="d-flex align-items-center flex-nowrap">
                ...
                <!-- Header Logo -->
                <div class="col-auto pl-0 pr-1 pr-md-3 d-flex align-items-center">
                    <a class="d-block py-20px mr-2 mr-md-3 ml-0 d-flex align-items-center" href="{{ route('home') }}">
                        @php
                ...
                <div class="d-lg-none ml-auto mr-0">
                    <a class="p-1 d-block text-reset" href="javascript:void(0);" data-toggle="class-toggle"
                ...
                @if (Auth::check() && auth()->user()->user_type == 'customer')
                    ...
                    <!-- Wishlist -->
                    <div class="mr-0 mr-lg-3 ml-2 ml-md-4">
                        <div class="" id="wishlist">
                            @include('frontend.partials.wishlist')
                        </div>
                    </div>
                    <!--add to cart-->
                   <div class="d-lg-none ml-2 flex-shrink-0">
                        <a href="{{ route('cart') }}">
                        <svg xmlns="http://www.w3.org/2000/svg" 
                          ...
                          <path d="M7 4H3v2h2l2.6 9.6c.2.8.9 1.4 1.8 1.4h7.8c.9 0 1.6-.6 1.8-1.4L21 8H8.2L7 4z"/>
               ...
               <div class="ml-2 ml-lg-auto mr-0 flex-shrink-0">
                    @auth
                        <span class="d-flex align-items-center nav-user-info py-20px @if (isAdmin()) ml-5 @endif"
                            id="nav-user-info">
                            <!-- Image -->
                            <span class="size-20px size-lg-40px  rounded-circle overflow-hidden border border-transparent nav-user-img">
                                @if ($user->avatar_original != null)
                                    <img src="{{ uploaded_asset(Auth::user()->avatar_original) }}" class="img-fit h-100 mb-2" alt="{{ translate('avatar') }} "
                                        onerror="this.onerror=null;this.src='{{ static_asset('assets/img/avatar-place.png') }}';">
                                @else
                                    <img src="{{ static_asset('assets/img/avatar-place.png') }}" class="image mb-2"
                                        alt="{{ translate('avatar') }}"
                                        onerror="this.onerror=null;this.src='{{ static_asset('assets/img/avatar-place.png') }}';">
                                @endif
                            </span>
                            <!-- Name -->
                            <h4 class="h5 fs-14 fw-700 ml-2 mb-0 middle-text-color-visibility d-none d-lg-block" style="color: {{ $middleHeaderTextColor }}">{{ $user->name }}</h4>
                        </span>
                    @else
                        <!--Login & Registration -->
                ...
                <div class="position-static float-right">
                    <div class="aiz-user-top-menu bg-white rounded-0 border-top shadow-sm" style="width:220px;margin-top: -40px !important;">
                            ...
                            @elseif(optional(Auth::user())->user_type == 'affiliate')
                                <li class="user-top-nav-element border border-top-0" data-id="1">
                                    <a href="{{ route('affiliate.user.index') }}"
                                        ...
                                        <span
                                            class="user-top-menu-name has-transition ml-3">{{ translate('Affiliated Dashboard') }}</span>
                                    </a>
                                </li>
                            @else
                        ...
                        <li class="user-top-nav-element border border-top-0">
                            @php
                                $referralLink = url('users/registration?referral_code=' . auth()->user()->referral_code);
                                $whatsappText = urlencode('Join using my referral link: ' . $referralLink);
                            @endphp
                            ...
                            <a href="https://wa.me/?text={{ $whatsappText }}"
                               ...
                               class="text-truncate text-dark px-4 fs-14 d-flex align-items-center hov-column-gap-1">
                                ...
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#b5b5bf" viewBox="0 0 24 24">
                                    <path d="M18 16.08c-.76 0-1.44-.3-1.96-.77l-7.12 3.56a2 2 0 1 1-.9-1.78l7.12-3.56a2 2 0 1 1 0-2.98l-7.12-3.56a2 2 0 1 1 .9-1.78l7.12 3.56A2 2 0 1 1 18 16.08z"/>
                                </svg>
    ...
    @if(isCustomer())
        <li class="user-top-nav-element border border-top-0">
            <a href="{{ route('agent.join') }}"
               class="text-truncate text-dark px-4 fs-14 d-flex align-items-center hov-column-gap-1">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#b5b5bf" viewBox="0 0 24 24">
                    <path d="M12 12c2.7 0 4.8-2.1 4.8-4.8S14.7 2.4 12 2.4 7.2 4.5 7.2 7.2 9.3 12 12 12zm0 2.4c-3.2 0-9.6 1.6-9.6 4.8v2.4h19.2v-2.4c0-3.2-6.4-4.8-9.6-4.8z"/>
                </svg>
                <span class="user-top-menu-name has-transition ml-3">Membership</span>
            </a>
        </li>
    @endif
@endauth
                            ...
                            <ul class="list-inline mb-0 pl-0 hor-swipe c-scrollbar-light">
                                @if (get_setting('header_menu_labels') != null)
                                    @foreach (json_decode(get_setting('header_menu_labels'), true) as $key => $value)
                                        ...
                                        @if ($value == 'Membership Retainer' && !auth()->check())
                                            @continue
                                        @endif
