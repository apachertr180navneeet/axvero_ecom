        ...
        <div class="card-body">
            <div class="row gutters-6">
                ...
                @if ($order->seller_id == $admin_user_id || get_setting('product_manage_by_admin') == 1)
                    ...
                    @endphp
                    @if (empty($order->shiprocket_shipment_id) && in_array($order->delivery_status, ['pending', 'confirmed']))
                        <div class="col-md-2 ml-auto">
                            <label class="d-block">&nbsp;</label>
                            <button type="button" class="btn btn-primary btn-block create-shiprocket-shipment" data-order-id="{{ $order->id }}">
                                <i class="las la-shipping-fast"></i> {{ translate('Create Shiprocket Shipment') }}
                            </button>
                        </div>
                    @endif
                    <div class="col-md-2 ml-auto">
                        ...
                        @if ($order->delivery_status == 'pending' || $order->delivery_status == 'confirmed')
                            ...
                            @else
                                <select class="form-control aiz-selectpicker" id="select_shipping_info" name="shipping_system" disabled>
                                    <option value="">
                                        {{ translate('Create shipment to assign') }}
                                    </option>
                                </select>
                            @endif
                        @else
...
@section('script')
    ...
    <script type="text/javascript">
        ...
        function loadShippingInfoForShiprocket() {
            ...
            $.when(
                        ...
                        html += `
                            <div class="border p-3 mb-3 rounded">
                                <div class="form-check">
                                    <input class="magic-radio" type="radio" name="pickup_location" value="${addr.pickup_location}" id="addr_${index}" ${checked}>
                                    <label class="form-check-label" for="addr_${index}">
                                        ${label}
                ...
                if (boxSizes.length > 0) {
                    boxSizes.forEach((box, index) => {
                        ...
                        html += `
                            <div class="border p-3 mb-3 rounded">
                                <div class="form-check">
                                    <input class="magic-radio shiprocket-box-option" type="radio" name="box_size_id" value="${box.id}" id="box_${box.id}" data-length="${box.length}" data-breadth="${box.breadth}" data-height="${box.height}" ${index === 0 ? 'checked' : ''}>
                    ...
                    html += `
                        <div class="border p-3 mb-3 rounded">
                            <div class="form-check">
                                <input class="magic-radio shiprocket-box-option" type="radio" name="box_size_id" value="" id="box_default" data-length="${defaultBoxDimensions.length}" data-breadth="${defaultBoxDimensions.breadth}" data-height="${defaultBoxDimensions.height}" checked>
                                <label class="form-check-label" for="box_default">
                ...
                $('#address-list').html('<p class="text-danger">' + message + '</p>');
            });
        }

        $(document).ready(function () {
            $(document).on('click', '.create-shiprocket-shipment', function () {
                loadShippingInfoForShiprocket();
