...
class OrderController extends Controller
{
    ...
    public function shiprocket_tracking($id)
    {
        $order = Order::where('id', $id)->where('user_id', auth()->user()->id)->first();
        if (!$order) {
            return response()->json([
        ...
        if ($order->shipping_method !== 'shiprocket') {
            return response()->json([
                'result' => false,
                'message' => translate('This order is not shipped via Shiprocket')
            ], 422);
        }
        ...
        try {
            ...
            $tracking = $order->shiprocket_shipment_id
                ? $shiprocket->trackShipment((int) $order->shiprocket_shipment_id, $order->delivery_status)
            ...
            $currentStatus = (string) ($trackingData['shipment_track'][0]['current_status'] ?? 'unknown');

            $mappedDeliveryStatus = $this->mapShiprocketStatusToDelivery($shipmentStatus, $order->delivery_status);

            $order->shiprocket_status = strtolower(str_replace(' ', '_', $currentStatus));
            $order->shiprocket_status_code = $shipmentStatus;
            $order->delivery_status = $mappedDeliveryStatus;
            $order->save();

            return response()->json([
                'result' => true,
                'message' => translate('Tracking fetched successfully'),
                'tracking' => $tracking,
                'order' => [
                    'id' => $order->id,
                    'code' => $order->code,
                    ...
                    'shiprocket_status_code' => $order->shiprocket_status_code,
    ...
    protected function mapShiprocketStatusToDelivery(int $shipmentStatus, string $currentStatus): string
    {
