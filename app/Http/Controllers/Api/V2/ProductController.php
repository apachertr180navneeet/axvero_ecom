...
class ProductController extends Controller
{
    ...
    public function product_details($slug, $user_id)
    {
        ...
        if (
            get_setting("last_viewed_product_activation") == 1 &&
            $user_id != null
        ) {
    ...
    public function seller($id, Request $request)
    {
        ...
        $products = Product::where("added_by", "seller")->where(
            "user_id",
            $shop->user_id,
        );
    ...
    public function brand($slug, Request $request)
    {
        ...
        return new ProductMiniCollection(
            filter_products($products)->latest()->paginate(10),
        );
    }
    ...
    public function todaysDeal()
    {
        ...
        return new ProductMiniCollection(
            filter_products($products)->limit(20)->latest()->get(),
        );
    }
    ...
    public function inhouse()
    {
        ...
        return new ProductMiniCollection(
            filter_products($products)->latest()->paginate(12),
        );
    }
    ...
    public function bestSeller()
    {
        ...
        return new ProductMiniCollection(
            filter_products($products)->limit(20)->get(),
        );
    }
    ...
    public function topFromSeller($slug)
    {
        ...
        $products = Product::where("user_id", $product->user_id)
            ...
            ->physical();
        return new ProductMiniCollection(
            filter_products($products)->limit(10)->get(),
        );
    }

    public function newFromSeller($id)
    {
        $shop = Shop::findOrFail($id);
    ...
    public function search(Request $request)
    {
        ...
        if (!empty($category_ids)) {
            ...
            foreach ($category_ids as $cid) {
                $n_cid = array_merge(
                    $n_cid,
                    CategoryUtility::children_ids($cid),
                );
            }
            ...
            $products->where(function ($query) use ($name) {
                foreach (explode(" ", trim($name)) as $word) {
                    $query
                        ...
                        ->orWhereHas("product_translations", function (
                            $query,
                        ) use ($word) {
        ...
        return new ProductMiniCollection(
            filter_products($products)->paginate(10),
        );
    }
