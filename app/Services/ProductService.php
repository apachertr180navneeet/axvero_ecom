...
class ProductService
{
    public function store(array $data)
    {
        ...
        // $collection['added_by'] = $data['product_type'];

        // if ($data['product_type'] == 'seller') {
    ...
    public function update(array $data, Product $product)
    {
        $collection = collect($data);
            ...
            if (!empty($data['seller_id'])) {
                $user_id = (int) $data['seller_id'];
                $added_by = 'seller';
            }
        
            $collection['user_id'] = $user_id;
            $collection['added_by'] = $added_by;
        ...
        if ($collection['shipping_description'] == null) {
            $collection['shipping_description'] = strip_tags($collection['shipping_description']);
        }
        
        if ($collection['return_description'] == null) {
            $collection['return_description'] = strip_tags($collection['return_description']);
        }
...
//     }
                ...
                if (empty($collection['meta_description'])) {
                    $collection['meta_description'] = isset($collection['description'])
                        ? strip_tags($collection['description'])
                        : '';
                }
            
                if (empty($collection['meta_img'])) {
                    $collection['meta_img'] = $collection['thumbnail_img'] ?? null;
                }
                ...
                $attributes = isset($collection['choice_no'])
                    ? json_encode($collection['choice_no'])
                    : json_encode([]);
