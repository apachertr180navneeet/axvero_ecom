...
Route::controller(VerificationController::class)->group(function () {
    ...
    Route::post('/email/verify-code', 'verifyCode')->name('verification.code.confirm');
...
Route::controller(HomeController::class)->group(function () {
    ...
    Route::post('/recent-products', 'getRecentProducts');
...
Route::post('/agent/payment/success', [AgentJoinController::class, 'paymentSuccess'])
    ->name('agent.payment.success');

Route::post('/agent/payment/fail', [AgentJoinController::class, 'paymentFail'])
    ->name('agent.payment.fail');

Route::post('/agent/payment/webhook', [AgentJoinController::class, 'paymentWebhook'])
    ->name('agent.payment.webhook');
...
//SSLCOMMERZ END
Route::get('/payu/p', [PayuController::class, 'pay'])->name('payu.pay');
Route::post('/payu/success', [PayuController::class, 'success'])->name('payu.success');
Route::post('/payu/fail', [PayuController::class, 'fail'])->name('payu.fail');

Route::post('/set-bulk-cod', function () {
    session(['bulk_cod' => true]);
    return response()->json(['status' => true]);
})->name('set.bulk.cod');
Route::post('/cart/update-bulk-buyer', [CartController::class, 'updateBulkBuyer'])
    ->middleware('auth')
    ->name('cart.update.bulk-buyer');
...
Route::controller(CheckoutController::class)->group(function () {
    ...
    Route::post('/updateDeliveryInfo', 'updateDeliveryInfo')->name('checkout.updateDeliveryInfo');
     Route::post('/toggle-online-pay', 'toggleOnlinePay')->name('checkout.toggle-online-pay');
});
});
...
Route::group(['middleware' => ['auth']], function () {
            // Join as Agent - Form View
    Route::get('/join-as-agent', [AgentJoinController::class, 'create'])
        ->name('agent.join');

    
    Route::post('/join-as-agent/skip', [AgentJoinController::class, 'skipAgentJoin'])->name('agent.join.skip');
    Route::get('/join-agent/export', [AgentJoinController::class, 'export'])->name('join-agent.export');
    Route::post('/join-agent/import', [AgentJoinController::class, 'import'])->name('join-agent.import');

    // Join as Agent - Save Data
    Route::post('/join-as-agent/store', [AgentJoinController::class, 'store'])->name('agent.join.store');

    Route::post('/agent/payment/initiate', [AgentJoinController::class, 'initiatePayment'])
    ->name('agent.payment.initiate');
...
Route::get('/affiliate/login', [App\Http\Controllers\HomeController::class, 'login'])->name('affiliate.login');
Route::get('/affiliate/register', [App\Http\Controllers\HomeController::class, 'registration'])->name('affiliate.registration');
